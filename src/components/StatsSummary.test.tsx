...
describe('StatsSummary loading indication', () => {
  const games = [makeGame(1, 100, 90)]

  it('does not show a refreshing indicator when not loading', () => {
    render(<StatsSummary games={games} loading={false} />)

    expect(screen.queryByText('Refreshing…')).not.toBeInTheDocument()
    const section = screen.getByText('Avg combined points').closest('section')
    expect(section).toHaveAttribute('aria-busy', 'false')
  })

  it('shows a refreshing indicator and dims content while loading, but keeps showing stale data', () => {
    render(<StatsSummary games={games} loading={true} />)

    expect(screen.getByText('Refreshing…')).toBeInTheDocument()
    // Stale data (from the previous window) stays visible during refetch.
